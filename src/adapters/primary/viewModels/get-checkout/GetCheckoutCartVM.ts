...
import { CartItemVM, getProductsInCart, createCartItemsVMFromCartItems } from '../get-cart/getCartVM';
...
export class GetCheckoutCartVM {
  ...
  get vm(): CheckoutCartVM {
    ...
    if (this.totalWithPromotion != this.total) {
      res.subTotalWithPromotion = this.formatter.format(this.totalWithPromotion / 100);
    }
    return res;
  }
